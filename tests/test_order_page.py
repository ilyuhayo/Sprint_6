...
class TestOrderPage:
    def test_fill_order_form_case_1(self, driver):
        ...
        order_page.find_element_located((By.XPATH, f"//div[text()='{data['metro_station']}']")).click()
        order_page.set_phone_number(data["phone_number"])
        order_page.click_next_button()
        order_page.select_delivery_day()
        order_page.find_element_located(
            (By.XPATH, f"//div[@class='react-datepicker__day react-datepicker__day--{data['delivery_day']}']")).click()
        order_page.select_rental_period()
        order_page.find_element_located((By.XPATH, f"//div[text()='{data['rental_period']}']")).click()
